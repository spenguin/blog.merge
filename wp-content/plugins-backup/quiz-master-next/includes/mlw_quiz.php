	...
	if (!isset($_POST["complete_quiz"]) && $mlw_quiz_options->quiz_name != "" && $mlw_qmn_isAllowed)
	{
		...
		if ($mlw_quiz_options->pagination != 0)
		{
			...
			<script type="text/javascript">
				...
				{
					var answer = jQuery( '.mlw_qmn_quiz div:visible input[type=radio]:checked' ).val();
					var control = jQuery( '.mlw_qmn_quiz div.slide2:visible').html();
					
					if( control != undefined ) {
						window.start_validation = 1;
					}
					
					if( window.start_validation != undefined && answer === undefined ) {
		...
		$mlw_display .= "<span class='mlw_qmn_message_before' style='line-height:24px;'>".$mlw_message_before."</span><br />";
		$mlw_display .= "<img src='/blog/wp-content/library/2014/07/ppc_quiz_header.png' /><br />";
		...
		foreach($mlw_questions as $mlw_question) {
			...
			$mlw_display .= "<span class='question_no'></span>";
			...
			$mlw_display .= "<span class='header_ppc'>".$mlw_message_before."</span><br />";
			$mlw_display .= "<img src='/blog/wp-content/library/2014/07/ppc_quiz_header.png' /><br />";
			$mlw_display .= "<p style='font-weight:lighter;'>Question <strong>" . $mlw_question->question_id . "</strong> of <strong>16</strong> </p>";
			
			if ($mlw_quiz_options->question_numbering == 1) { $mlw_display .= $mlw_qmn_total_questions.") "; }
			$mlw_display .= htmlspecialchars_decode($mlw_question->question_name, ENT_QUOTES)."</span><br /><br />";
		...
		foreach($mlw_questions as $mlw_question) {
			...
			{	
				if ( $mlw_question->question_type == 0 || $mlw_question->question_type == 1 || $mlw_question->question_type == 2)
				{
					...
					{
						//echo "<pre>"; var_dump($mlw_qmn_question_answers_each);
						if (htmlspecialchars(stripslashes($mlw_user_answer), ENT_QUOTES) == esc_attr($mlw_qmn_question_answers_each[0]))
						{
							...
							if ($mlw_qmn_question_answers_each[2] == 1)
							{
								...
								$msg .= $idx . " - " . $mlw_qmn_question_answers_each[0] . "<br/>";
							}
						}
