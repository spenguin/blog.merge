<?php
	...
	if ($anchor != "") {
		...
		$tweet['text'] = preg_replace("/(https:\/\/)(.*?)\/([\w\.\/\&\=\?\-\,\:\;\#\_\~\%\+]*)/", "<a href=\"\\0\" rel=\"external nofollow\">".$anchor."</a>", $tweet['text']);
	} else {
		$tweet['text'] = preg_replace("/(http:\/\/)(.*?)\/([\w\.\/\&\=\?\-\,\:\;\#\_\~\%\+]*)/", "<a href=\"\\0\" rel=\"external nofollow\">\\0</a>", $tweet['text']);
		$tweet['text'] = preg_replace("/(https:\/\/)(.*?)\/([\w\.\/\&\=\?\-\,\:\;\#\_\~\%\+]*)/", "<a href=\"\\0\" rel=\"external nofollow\">\\0</a>", $tweet['text']);
}
	if ($hashlinks=="yes") {
	...
	if ($timeline == "yes") {
		...
		if ($options['linktotweet'] == "yes") {
			$posted = '<a href="http://twitter.com/'.$tweet['user']['screen_name'].'/statuses/'.$tweet['id'].'" rel="external nofollow">'.$posted.'</a>';
		}
	}
}
